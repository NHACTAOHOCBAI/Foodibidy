...
const Order = () => {
    ...
    const filteredOrders = useMemo(() => {
        ...
        return orders.filter((order) =>
            ...
            (order.restaurant?.restaurantName || '').toLowerCase().includes(searchText.toLowerCase())
        );
    }, [orders, searchText]);
        ...
        <>
            ...
            >
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    Search: <Input style={{ width: 400 }} value={searchText} onChange={handleSearchChange} placeholder="Search by customer or restaurant name" />
                </div>
            ...
            <Table<Order>
                ...
                expandable={{
                    expandedRowRender: (record) => (
                        <div>
                            <p style={{ margin: 0 }}>Shipper phone: {record.shipperPhone || 'N/A'}</p>
                            <p style={{ margin: 0 }}>Shipper name: {record.shipperName || 'N/A'}</p>
                        </div>
                    ),
                    rowExpandable: (record) => record.shipperName !== undefined || record.shipperPhone !== undefined,
                }}
                rowKey="id"
                pagination={{
                    pageSize: 4,
