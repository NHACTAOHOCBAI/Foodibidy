    ...
    try {
        ...
        const ordersTrend = past5Days.map(day => {
            ...
            const ordersInDay = orders.filter(o =>
                moment(o.createdAt).isSameOrAfter(day) && moment(o.createdAt).isBefore(nextDay)
            )
            return {
...
export const getRestaurantDashboard = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        // Tính tổng doanh thu
        const totalRevenue = (
            await Promise.all(completedOrders.map(calculateOrderTotal))
        ...
        const revenueTrend = await Promise.all(
            past5Days.map(async day => {
                ...
                const ordersInDay = completedOrders.filter(o =>
                    moment(o.createdAt).isSameOrAfter(day) && moment(o.createdAt).isBefore(nextDay)
                )
                const total = (
                    await Promise.all(ordersInDay.map(calculateOrderTotal))
