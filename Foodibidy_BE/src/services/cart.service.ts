...
import { CART_MESSAGES, DISH_MESSAGES } from '~/constants/messages'
...
class CartService {
  ...
  async addDishToCart(userId: string, Dish: AddDishToCart) {
    const doc = (await this.cartCollection.where('userId', '==', userId).get()).docs[0]
    ...
    try {
      ...
      await this.cartCollection.doc(doc.id).update(updatedCart)
  ...
  async getCart(id: string) {
    ...
    throw new ErrorWithStatus({ message: CART_MESSAGES.CART_NOT_FOUND, status: HTTP_STATUS.CREATED })
  }
